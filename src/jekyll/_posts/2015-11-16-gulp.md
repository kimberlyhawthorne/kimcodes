...
But, despite the extra task of restructuring my whole website, setting up Gulp was more enjoyable than I expected. It made <a href="http://www.kimcod.es" target="_blank">www.kimcod.es</a> better constructed and will make my remaining 48 projects easier to develop.
...
* A <strong>task</strong>, or the name of a function that can be run from the terminal.
...
* A <strong>pipe</strong> is the means by which data is streamed to be processed. Or more crudely, the stream files are dumped into a plugin, then another plugin, then another plugin, until all processing is completed and the modified stream can be dumped in its final destination.


### What My Gulpfile Does
I'll give just a few examples here, rather than going through the entire file. You'll get the gist.
<pre>
<code class='language-javascript'> 
//	javascript
...
####Jekyll
This tasks runs after jekyll is built via its own, baked-in build system. It simply copies the finalized Jekyll website files to the destination.

####Watch
I've also included a ```gulp watch``` task to watch for changes in any JavaScript, CSS, HTML or Jekyll files, and run the corresponding task after changes are detected.


With this much-needed task behind me, I leave you with the wise words of Lloyd Christmas: "Big Gulps, huh? Welp, see ya later."